...
<body>
	...
	<div>
		<table class="layui-table">
			...
			<thead>
				<tr>
					<th style="text-align: center;">事故可能性</th>
					<th style="text-align: center;">事故严重性</th>
					<th style="text-align: center;">事故风险</th>
			...
			<tbody id="paramsInfoDiv">
				<c:forEach items="${pis }" var="item">
					<tr>
						<td><input type="text" class="form-control" id="pi21_${item.id }" value="${item.pi21 }"></td>
						<td><input type="text" class="form-control" id="pi22_${item.id }" value="${item.pi22 }"></td>
						<td><input type="text" class="form-control" id="pi23_${item.id }" value="${item.pi23 }"></td>
				...
				<tr id="addInfo" style="display:none;">
					<td><input type="text" class="form-control" id="pi21"></td>
					<td><input type="text" class="form-control" id="pi22"></td>
					<td><input type="text" class="form-control" id="pi23"></td>
					<td style='text-align: center;'>
				...
				<tr id='addBtn'>
					<td colspan="4"><button type="button" class="layui-btn layui-btn-fluid" onclick="addShow()"><i class="layui-icon layui-icon-add-1"></i> 添加信息</button></td>
				</tr>
			</tbody>
		</table>
	</div>
	<script type="text/javascript">
		...
		function doAdd(){
			...
			var pi21 = $("#pi21").val().trim();
			var pi22 = $("#pi22").val().trim();
			var pi23 = $("#pi23").val().trim();
			$.ajax({
				type : "post",
		...
		function doEdit(id){
			...
			var pi21 = $("#pi21_"+id).val().trim();
			var pi22 = $("#pi22_"+id).val().trim();
			var pi23 = $("#pi23_"+id).val().trim();
			$.ajax({
				type : "post",
