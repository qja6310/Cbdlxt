...
<body>
			...
			<div style="position: absolute; left: 310px;height: 850px;width: 1050;">
				<div style="border: 1px solid #5FB878;">
					...
					<div class="layui-card-body">
						<select class="form-control" id="sixFunctionSelect" onchange="sixFunctionParams()">
							<option value="">请选择船舶装备模型</option>
						</select>
					</div>
				</div>
				<div style="border: 1px solid #5FB878;margin-top: 5px;">
					<div class="layui-card-header" style="font-size: 20px;font-weight: 600;background-color: #5FB878;">
						参数输入
						<label style="margin-left: 800px;color: white;" onclick="editData()">编辑数据</label>
					</div>
					<div class="layui-card-body" id="sixFunctionBody">
	...
	<script type="text/javascript">
		...
		function showSixFunction(list){
			var sixFunctionSelect = $("#sixFunctionSelect");
			sixFunctionSelect.html("");
			if(list.length > 0){
				for(var i = 0; i < list.length; i++){
					var option = $("<option value='"+list[i].funCode+"'>"+list[i].funName+"</option>");
					sixFunctionSelect.append(option);
				}
				sixFunctionParams();
			}else{
				var option = $("<option>该模型未配置六性</option>");
				sixFunctionSelect.append(option);
			}
		}
		
		//显示六性参数
		function sixFunctionParams(){
			var funCode = $("#sixFunctionSelect").val();
			...
			$('#sixFunctionBody').load('<%=request.getContextPath() %>/sixFunction/details?taskModelId='+taskModelId+'&jcmId='+jcmId+'&funCode='+funCode);
		}
		...
		function editData(){
			...
			if(funCode != '' && funCode != undefined){
				layer.open({
		        	...
		        	content: '<%=request.getContextPath()%>/sixFunction/editData?taskModelId='+taskModelId+'&jcmId='+jcmId+'&funCode='+funCode,
		        	closeBtn:2,
		       });
			}
	    }
